...
pub fn build_streamer(
    ...
    if cfg!(windows) {
        ...
        // copy libvpl.dll if available
        let vpl_dll_path = afs::deps_dir().join("windows/libvpl/alvr_build/bin/libvpl.dll");
        if vpl_dll_path.exists() {
            sh.copy_file(vpl_dll_path, build_layout.openvr_driver_lib_dir())
                .unwrap();
        }
