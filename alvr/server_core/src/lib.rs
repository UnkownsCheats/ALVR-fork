...
impl ServerCoreContext {
    ...
    pub fn get_base_stations(&self) -> Vec<(u64, Pose)> {
        self.connection_context
            .tracking_manager
            .read()
            .get_base_stations()
    ...
    pub fn get_grayscreen_intensity(&self) -> f32 {
        let tracking_manager = self.connection_context.tracking_manager.read();
        tracking_manager.get_grayscreen_intensity(&SESSION_MANAGER.read().settings().headset)
    }
