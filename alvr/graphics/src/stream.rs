...
const GRAYSCREEN_INTENSITY_OFFSET: u32 = CK_CHANNEL2_CONST_OFFSET + VEC4_SIZE;
const PUSH_CONSTANTS_SIZE: u32 = GRAYSCREEN_INTENSITY_OFFSET + FLOAT_SIZE;
...
impl StreamRenderer {
    ...
    pub fn render(
        ...
        for (view_idx, view_params) in view_params.iter().enumerate() {
            ...
            render_pass.set_push_constants(
                ShaderStages::VERTEX_FRAGMENT,
                GRAYSCREEN_INTENSITY_OFFSET,
                &grayscreen_intensity.to_le_bytes(),
            );
