            ...
            match event {
                ...
                ServerCoreEvent::Tracking { poll_timestamp } => {
                    ...
                    if let Some(context) = &*SERVER_CORE_CONTEXT.read() {
                        ...
                        let ffi_base_station_motions = {
                            let base_stations = context.get_base_stations();
                            base_stations
                                .iter()
                                .map(|(id, pose)| {
                                    tracking::to_ffi_motion(
                                        *id,
                                        alvr_common::DeviceMotion {
                                            pose: *pose,
                                            linear_velocity: alvr_common::glam::Vec3::ZERO,
                                            angular_velocity: alvr_common::glam::Vec3::ZERO,
                                        },
                                    )
                                })
                                .collect::<Vec<_>>()
                        ...
                        let grayscreen_intensity = context.get_grayscreen_intensity();
                        ...
                        unsafe {
                            SetTracking(
                                ...
                                ffi_base_station_motions.as_ptr(),
                                ffi_base_station_motions.len() as i32,
                                grayscreen_intensity,
                            )
                        };
                    }
                }
