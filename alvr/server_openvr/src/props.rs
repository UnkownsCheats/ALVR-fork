...
use alvr_common::{
    BASE_STATION_1_ID, BASE_STATION_2_ID, BASE_STATION_3_ID, BASE_STATION_4_ID, BODY_CHEST_ID,
    ...
    HAND_TRACKER_LEFT_ID, HAND_TRACKER_RIGHT_ID, HEAD_ID, debug, settings_schema::Switch, warn,
};
use alvr_session::{
...
fn serial_number(device_id: u64) -> String {
    ...
    } else if device_id == *BASE_STATION_1_ID {
        "ALVR Base Station 1".into()
    } else if device_id == *BASE_STATION_2_ID {
        "ALVR Base Station 2".into()
    } else if device_id == *BASE_STATION_3_ID {
        "ALVR Base Station 3".into()
    } else if device_id == *BASE_STATION_4_ID {
        "ALVR Base Station 4".into()
    } else {
        "Unknown".into()
    }
}
...
pub extern "C" fn set_device_openvr_props(instance_ptr: *mut c_void, device_id: u64) {
        ...
        || device_id == *HAND_TRACKER_RIGHT_ID
    {
        ...
        || device_id == *BASE_STATION_2_ID
        || device_id == *BASE_STATION_3_ID
        || device_id == *BASE_STATION_4_ID
    {
        // Base station properties
        let settings = alvr_server_core::settings();
        let base_station_config = settings.headset.base_station_emulation.as_option();

        if let Some(config) = base_station_config {
            let version = match config.version {
                alvr_session::BaseStationVersion::V1 => "1.0",
                alvr_session::BaseStationVersion::V2 => "2.0",
            };

            set_prop(TrackingSystemNameString, "lighthouse");
            set_prop(ModelNumberString, format!("Vive Base Station {}", version).as_str());
            set_prop(ManufacturerNameString, "HTC");
            set_prop(RenderModelNameString, "{htc}/vr_tracker_vive_1_0");
            set_prop(ControllerTypeString, "vive_tracker_camera");
            set_prop(SerialNumberString, device_serial);
            set_prop(RegisteredDeviceTypeString, "ALVR/base_station");
            set_prop(DeviceClassInt32, "1"); // TrackingReference
            set_prop(WillDriftInYawBool, "false");
            set_prop(HasDisplayComponentBool, "false");
            set_prop(HasCameraComponentBool, "true");
            set_prop(HasDriverDirectModeComponentBool, "false");
            set_prop(HasVirtualDisplayComponentBool, "false");
            set_prop(DeviceIsWirelessBool, "true");
            set_prop(DeviceIsChargingBool, "false");
            ...
            set_icons("{htc}/icons/base_station");
        }
    }
}
