...
fn main() {
    ...
    // Exclude VPL encoder files if VPL library is not available
    let vpl_available = if platform_name == "windows" {
        let vpl_path = alvr_filesystem::deps_dir().join("windows/libvpl/alvr_build");
        vpl_path.join("include").exists() && vpl_path.join("lib").exists()
    } else {
        false
    };

    let source_files_paths = source_files_paths.filter(|path| {
        if !vpl_available {
            let path_str = path.to_string_lossy();
            !path_str.contains("VideoEncoderVPL")
        } else {
            true
        }
    });
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if vpl_include_path.exists() && vpl_lib_path.exists() {
            println!(
            ...
            println!("cargo:warning=VPL library not found, skipping VPL encoder support");
        }
    }
