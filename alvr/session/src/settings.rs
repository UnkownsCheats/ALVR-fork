...
Because of runtime limitations, this option is ignored when body tracking is active."
    ...
    #[schema(strings(
        display_name = "Head Snapping / Grayscreening",
        help = "When head rotation speed exceeds threshold, applies random rotation and grayscreen effect."
    ))]
    pub head_snapping: Switch<HeadSnappingConfig>,

    #[schema(strings(
        display_name = "Base Station Emulation",
        help = "Emulate base stations that show up in VR and SteamVR"
    ))]
    pub base_station_emulation: Switch<BaseStationEmulationConfig>,

    #[schema(strings(
        display_name = "Physical Space Amplification",
        help = "Boost virtual movement speed based on real-world movement"
    ))]
    pub psa: Switch<PSAConfig>,
}

#[derive(SettingsSchema, Serialize, Deserialize, Clone, Copy, PartialEq)]
#[schema(gui = "button_group")]
pub enum GrayScreenMode {
    #[schema(strings(display_name = "Vive"))]
    Vive,
    #[schema(strings(display_name = "Pimax"))]
    Pimax,
    #[schema(strings(display_name = "Rifts"))]
    Rifts,
}

#[derive(SettingsSchema, Serialize, Deserialize, Clone, PartialEq)]
#[schema(collapsible)]
pub struct HeadSnappingConfig {
    #[schema(strings(
        display_name = "Enable",
        help = "Enable head snapping and grayscreening effect"
    ))]
    #[schema(flag = "real-time")]
    pub enable: bool,
    ...
    #[schema(strings(
        display_name = "Mode",
        help = "Grayscreen effect mode: Vive (random rotation), Pimax (spinning), Rifts (backward PSA)"
    ))]
    pub mode: GrayScreenMode,

    #[schema(strings(
        display_name = "Rotation threshold",
        help = "Angular velocity threshold in degrees per second that triggers the effect"
    ))]
    #[schema(gui(slider(min = 10.0, max = 1000.0, step = 10.0)), suffix = "°/s")]
    pub rotation_threshold: f32,

    #[schema(strings(
        display_name = "Vive: Min rotation intensity",
        help = "Minimum random rotation offset in degrees (Vive mode)"
    ))]
    #[schema(gui(slider(min = 5.0, max = 90.0, step = 5.0)), suffix = "°")]
    pub vive_min_rotation: f32,

    #[schema(strings(
        display_name = "Vive: Max rotation intensity",
        help = "Maximum random rotation offset in degrees (Vive mode)"
    ))]
    #[schema(gui(slider(min = 5.0, max = 180.0, step = 5.0)), suffix = "°")]
    pub vive_max_rotation: f32,

    #[schema(strings(
        display_name = "Vive: Min rotation duration",
        help = "Minimum rotation duration in milliseconds (Vive mode)"
    ))]
    #[schema(gui(slider(min = 100, max = 2000, step = 100)), suffix = "ms")]
    pub vive_min_duration_ms: u64,

    #[schema(strings(
        display_name = "Vive: Max rotation duration",
        help = "Maximum rotation duration in milliseconds (Vive mode)"
    ))]
    #[schema(gui(slider(min = 100, max = 5000, step = 100)), suffix = "ms")]
    pub vive_max_duration_ms: u64,

    #[schema(strings(
        display_name = "Pimax: Min spin duration",
        help = "Minimum spin duration in milliseconds (Pimax mode)"
    ))]
    #[schema(gui(slider(min = 100, max = 2000, step = 100)), suffix = "ms")]
    pub pimax_min_duration_ms: u64,

    #[schema(strings(
        display_name = "Pimax: Max spin duration",
        help = "Maximum spin duration in milliseconds (Pimax mode)"
    ))]
    #[schema(gui(slider(min = 100, max = 5000, step = 100)), suffix = "ms")]
    pub pimax_max_duration_ms: u64,

    #[schema(strings(
        display_name = "Rifts: Backward PSA duration",
        help = "Duration of backward PSA effect in milliseconds (Rifts mode)"
    ))]
    #[schema(gui(slider(min = 100, max = 3000, step = 100)), suffix = "ms")]
    pub rifts_backward_duration_ms: u64,

    #[schema(strings(
        display_name = "Grayscreen intensity",
        help = "How much to desaturate the screen (0.0 = no effect, 1.0 = full grayscale)"
    ))]
    #[schema(gui(slider(min = 0.0, max = 1.0, step = 0.1)))]
    pub grayscreen_intensity: f32,

    #[schema(strings(
        display_name = "Current head rotation speed",
        help = "Current angular velocity of head rotation in degrees per second (read-only)"
    ))]
    #[schema(gui(slider(min = 0.0, max = 1000.0, step = 1.0)), suffix = "°/s")]
...
pub enum BaseStationVersion {
    #[schema(strings(display_name = "1.0"))]
    V1,
    #[schema(strings(display_name = "2.0"))]
    V2,
}

#[derive(SettingsSchema, Serialize, Deserialize, Clone, PartialEq)]
#[schema(collapsible)]
pub struct BaseStationEmulationConfig {
    #[schema(strings(
        display_name = "Enable",
        help = "Enable base station emulation"
    ))]
    #[schema(flag = "real-time")]
    pub enable: bool,

    #[schema(strings(
        display_name = "Base Station Version",
        help = "Choose between 1.0 or 2.0 base stations"
    ))]
    #[schema(flag = "steamvr-restart")]
    pub version: BaseStationVersion,

    #[schema(strings(
        display_name = "Number of Base Stations",
        help = "Number of base stations to emulate (1-4)"
    ))]
    #[schema(gui(slider(min = 1, max = 4, step = 1)))]
    pub count: u32,

    #[schema(strings(
        display_name = "Base Station 1 Position",
        help = "Position of first base station (x, y, z in meters)"
    ))]
    pub station1_position: [f32; 3],

    #[schema(strings(
        display_name = "Base Station 2 Position",
        help = "Position of second base station (x, y, z in meters)"
    ))]
    pub station2_position: [f32; 3],

    #[schema(strings(
        display_name = "Base Station 3 Position",
        help = "Position of third base station (x, y, z in meters)"
    ))]
    pub station3_position: [f32; 3],

    #[schema(strings(
        display_name = "Base Station 4 Position",
        help = "Position of fourth base station (x, y, z in meters)"
    ))]
    pub station4_position: [f32; 3],
}
...
pub struct PSAConfig {
    #[schema(strings(
        display_name = "Enable PSA",
        help = "Enable Physical Space Amplification to boost virtual movement speed"
    ))]
    #[schema(flag = "real-time")]
    pub enable: bool,

    #[schema(strings(
        display_name = "Movement boost multiplier",
        help = "How much to amplify virtual movement speed (1.0 = no boost, 2.0 = 2x speed, etc.)"
    ))]
    #[schema(gui(slider(min = 1.0, max = 10.0, step = 0.1)), suffix = "x")]
    pub boost_multiplier: f32,

    #[schema(strings(
        display_name = "Enable threshold-based PSA",
        help = "Only apply boost when moving faster than threshold speed"
    ))]
    #[schema(flag = "real-time")]
    pub enable_threshold: bool,

    #[schema(strings(
        display_name = "Movement threshold",
        help = "Minimum real-world movement speed in m/s to activate boost (when threshold-based is enabled)"
    ))]
    #[schema(gui(slider(min = 0.1, max = 5.0, step = 0.1)), suffix = "m/s")]
    pub movement_threshold: f32,

    #[schema(strings(
        display_name = "Current movement speed",
        help = "Current real-world movement speed in meters per second (read-only)"
    ))]
    #[schema(gui(slider(min = 0.0, max = 10.0, step = 0.1)), suffix = "m/s")]
...
pub fn session_settings_default() -> SettingsDefault {
    ...
    SettingsDefault {
        ...
        headset: HeadsetConfigDefault {
            ...
            head_snapping: SwitchDefault {
                enabled: false,
                content: HeadSnappingConfigDefault {
                    gui_collapsed: true,
                    enable: false,
                    mode: GrayScreenModeDefault {
                        variant: GrayScreenModeDefaultVariant::Vive,
                    },
                    rotation_threshold: 500.0,
                    vive_min_rotation: 15.0,
                    vive_max_rotation: 45.0,
                    vive_min_duration_ms: 500,
                    vive_max_duration_ms: 2000,
                    pimax_min_duration_ms: 500,
                    pimax_max_duration_ms: 2000,
                    rifts_backward_duration_ms: 1000,
                    grayscreen_intensity: 1.0,
                    current_rotation_speed: 0.0,
                },
            },
            base_station_emulation: SwitchDefault {
                enabled: false,
                content: BaseStationEmulationConfigDefault {
                    gui_collapsed: true,
                    enable: false,
                    version: BaseStationVersionDefault {
                        variant: BaseStationVersionDefaultVariant::V2,
                    },
                    count: 2,
                    station1_position: ArrayDefault {
                        gui_collapsed: true,
                        content: [0.0, 2.5, -2.0],
                    },
                    station2_position: ArrayDefault {
                        gui_collapsed: true,
                        content: [0.0, 2.5, 2.0],
                    },
                    station3_position: ArrayDefault {
                        gui_collapsed: true,
                        content: [-2.0, 2.5, 0.0],
                    },
                    station4_position: ArrayDefault {
                        gui_collapsed: true,
                        content: [2.0, 2.5, 0.0],
                    },
                },
