...
pub fn play_audio_loop(
    _is_running: impl Fn() -> bool,
    _device: &Device,
    _channels_count: u16,
    _sample_rate: u32,
    _config: AudioBufferingConfig,
    _receiver: &mut StreamReceiver<()>,
) -> Result<()> {
    // Audio playback not supported on this platform (e.g., Android uses its own audio system)
    Ok(())
}
